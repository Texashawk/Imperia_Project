...
namespace Tooltips
{
    public class CharacterTooltip : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private void FadeToolTip(bool fadeIn)
        {
            ...
            {
                intelColor = HelperFunctions.StringConversions.GetTextValueColor((int)cData.Intelligence);
                loyaltyColor = HelperFunctions.StringConversions.GetTextValueColor((int)cData.Honor);
        ...
        private void DrawToolTip()
        {
            ...
            if (!toolTipCreated)
            {
                ...
                if (cData.IntelLevel > 0)
                {
                    tooltipObject.transform.Find("Character Intelligence").GetComponent<Text>().text = HelperFunctions.StringConversions.ConvertCharacterValueToDescription((int)cData.Intelligence, cData.IntelLevel);                
                    tooltipObject.transform.Find("Character Loyalty").GetComponent<Text>().text = HelperFunctions.StringConversions.ConvertCharacterValueToDescription((int)cData.Honor, cData.IntelLevel);                
